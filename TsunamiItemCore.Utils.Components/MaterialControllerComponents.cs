...
namespace TsunamiItemCore.Utils.Components {
	...
	public class MaterialControllerComponents
	{
		...
		public class HGStandardController : MonoBehaviour
		{
			...
			public void Update()
			{
				if ((bool)Material)
				{
					if (Material.name != MaterialName && (bool)Renderer)
					{
						GrabMaterialValues();
					...
					if ((bool)_GreenChannelNormalTex)
					{
		...
		public class HGCloudRemapController : MonoBehaviour
		{
			...
			public void Update()
			{
				if ((bool)Material)
				{
					...
					Material.SetFloat("_InternalSimpleBlendMode", _InternalSimpleBlendMode);
		...
		{
			if ((bool)material && (bool)renderer)
			{
